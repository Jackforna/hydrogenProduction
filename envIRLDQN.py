...
class HRS_envIRL_DQN(gym.Env):

    def __init__(self):
        super(HRS_envIRL_DQN, self).__init__()
    ...
    def load_expert_trajectories(self, filename="expert_trajectories_DQN.csv"):
    ...
    def step(self, action):
        ...
        if isinstance(action, (int, np.integer)):
            action = list(map(int, format(action, '05b')))
        ...
        '''
        hydrogen_level = float(self.storage.actual_quantity)
        reward = self.learned_rewards.get((int(hydrogen_level), int(demand), int(loss_power)), -6)
    ...
    def save_expert_trajectories(self, expert_trajectories, filename="expert_trajectories_DQN.csv"):
    ...
    def generate_expert_trajectories(self, num_episodes=300):
        ...
        num_suboptimal = int(num_episodes * 0.2)

        for i in range(num_episodes):
            print(f"\nTraiettoria {'subottimale' if i < num_suboptimal else 'esperta'} n.{i}\n")
            self.state, _ = self.reset()
            self.state[0] = np.random.uniform(100, 400)
            self.state[1] = np.clip(np.random.normal(315, 135), 30, 600)  # energia prodotta
            self.state[4] = np.clip(np.random.normal(325, 275), 50, 600)  # domanda
            ...
            for _ in range(self.len_episodes):
                if i < num_suboptimal:
                    # Azione subottimale: simula una decisione random o bloccata
                    action = [0, 0, 0, 0, 0] if np.random.rand() < 0.5 else self.expert_policy()
                else:
                ...
                if n == 10:
                    # Inserisce anche loss_power e demand_remained reali
                    state_disc = self.discretize_state(summ / 10, int(self.demand_remained), int(self.loss_remained))
                    episode.append((state_disc, action))
                    n = 0
                    summ = 0
            expert_trajectories.append(episode)
        return expert_trajectories
    ...
    def maxent_irl(self, expert_trajectories, iterations=500, alpha=0.1):
        ...
        df.to_csv("learned_rewards_DQN.csv", index=False)